...
export default function StudentsPage() {
  ...
  const handleBulkWithToast = async () => {
    try {
      await handleBulk();
      show("Selected students updated.");
    } catch (err) {
    ...
    <div className="students-page">
      <div className="page-toolbar">
        <div className="saved-views">
          ...
          <button type="button" className="text-link" onClick={saveCurrentView}>
            Save current
          </button>
          {savedViews.map((view) => (
            <span key={view.name} className="saved-view">
        ...
        <div className="toolbar-actions">
          ...
          <button
            ...
            onClick={() => setArchivedView(!filters.includeDeleted)}
          >
            {filters.includeDeleted ? "Active Students" : "Archived Students"}
          </button>
          <button
            type="button"
            className="button button-secondary"
      ...
      {selectedIds.length > 0 && !filters.includeDeleted && (
        <div className="bulk-actions">
          ...
          >
            {(
              filterOptionsData?.data?.statuses ?? [
                "active",
                "inactive",
                "graduated",
                "transferred",
          ...
          <button
            ...
            onClick={handleBulkWithToast}
          >
            {bulkMutation.isPending ? "Updating..." : "Update Status"}
          </button>
          <button
            type="button"
            className="button button-secondary"
      ...
      {bulkMutation.isError && (
        <p className="inline-error">
          {getApiErrorMessage(bulkMutation.error, "Bulk update failed.")}
        </p>
      )}
      <StudentsContent
        isLoading={isLoading}
        ...
        showArchived={filters.includeDeleted}
        sortBy={filters.sortBy}
        sortOrder={filters.sortOrder}
      ...
      <DeleteStudentModal
        ...
        onClose={() => !deleteMutation.isPending && setStudentToDelete(null)}
      />
    </div>
  );
}
