...
export default function PromotionPage() {
  ...
  const selectedRows = useMemo(
    () =>
      sourceRows.filter((row) =>
        selected.includes(row.student?._id || row.student),
      ),
    [sourceRows, selected],
  );

  const selectedDestination = rooms.find(
    (room) => room._id === form.toClassroomId,
  );
  const capacityRemaining = selectedDestination?.capacity
    ? Math.max(
        0,
        selectedDestination.capacity - (selectedDestination.studentCount || 0),
      )
    : null;
  const exceedsCapacity =
    capacityRemaining !== null && selected.length > capacityRemaining;
  ...
  const toggleAll = () => {
    const ids = sourceRows
      .map((row) => row.student?._id || row.student)
      .filter(Boolean);
  ...
  const submit = async (event) => {
    ...
    if (
      ...
      setError(
        "Select the source academic year, destination academic year, and destination classroom.",
      );
      return;
    }
    if (!selected.length) {
    ...
    if (exceedsCapacity) {
      setError(
        "The selected students exceed the remaining classroom capacity.",
      );
      return;
    }
    ...
    if (
      ...
      setError(
        "Every selected student needs a unique positive destination roll number.",
      );
      return;
    }
    setPromoting(true);
    ...
    <main className="page">
      ...
      <section className="card">
        ...
        {selectedDestination && (
          <p className="muted-text">
            ...
            {selectedDestination.capacity
              ? `${selectedDestination.studentCount || 0}/${selectedDestination.capacity} currently enrolled; ${capacityRemaining} places remaining.`
      ...
      <section className="card">
              ...
              {sourceRows.map((row) => {
                ...
                return (
                  <tr key={row._id}>
                    ...
                    <td>
                      {row.classroom ? classroomLabel(row.classroom) : "—"}
                    </td>
                    <td>{row.rollNo}</td>
                    <td>
                      <input
                        ...
                        min="1"
                        disabled={!checked}
                        value={rollNumbers[id] ?? row.rollNo}
                        onChange={(e) =>
                          setRollNumbers((current) => ({
                            ...current,
                            [id]: e.target.value,
                          }))
                        }
        ...
        {selectedRows.length > 0 && (
          <p className="muted-text">
            Review destination roll numbers before promoting. The server
            performs final enrollment, capacity, and roll-number conflict
            checks.
          </p>
        )}
      </section>
