...
export const getStudentFilterOptions = asyncHandler(async (req, res) => {
  const options = await studentService.getStudentFilterOptions(req.query);
...
export const exportStudents = asyncHandler(async (req, res) => {
  const students = await studentService.getStudentsForExport(
    req.query,
    req.user,
  );
  const headers = [
...
export const importStudents = asyncHandler(async (req, res) => {
  ...
  res.status(201).json({
    success: true,
    message: `${students.length} student(s) imported successfully.`,
    data: students,
  });
});

export const restoreStudent = asyncHandler(async (req, res) => {
  const student = await studentService.restoreStudentById(
    req.params.id,
    req.requestId,
  );
  res.status(200).json({
...
export const getStudentAuditHistory = asyncHandler(async (req, res) => {
  const data = await studentService.getStudentAuditHistory(
    req.params.id,
    req.user,
  );
  res.status(200).json({ success: true, data });
