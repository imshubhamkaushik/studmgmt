...
const formatStudentId = (sequence) =>
  `STU-${String(sequence).padStart(6, "0")}`;

const inspect = async () => {
  const students = await Student.find()
    .select("studentId class section rollNo createdAt")
    .lean();
  ...
  for (const student of students) {
    ...
    const match =
      typeof student.studentId === "string"
        ? STUDENT_ID_PATTERN.exec(student.studentId)
        : null;
...
try {
  ...
  if (dryRun) {
    ...
    if (report.malformed || report.duplicateIds) {
      throw new Error(
        "Fix malformed or duplicate Student IDs before migration.",
      );
    }
