...
const buildStudentFilter = (query = {}) => {
  ...
  if (search) {
    ...
    filter.$or = ["name", "class", "section", "studentId", "admissionNo"].map(
      (field) => ({
        [field]: {
          $regex: escapedSearch,
          $options: "i",
    ...
    if (numericRollNo !== null) {
      filter.$or.push({ rollNo: numericRollNo });
    }
  }
...
export const createStudent = async (studentData, requestId = null) => {
  ...
  await writeAudit({
    ...
    action: "CREATE",
    changes: { after: student.toObject() },
    requestId,
  });
...
export const getStudents = async (query, user = null) => {
  ...
  const filter = await applyTeacherStudentScope(
    user,
...
export const getStudentFilterOptions = async (query = {}) => {
  const sectionFilter =
    typeof query.class === "string" && query.class.trim()
      ? { class: query.class.trim() }
      : {};
  
  const [classes, sections] = await Promise.all([
    Student.distinct("class"),
    Student.distinct("section", sectionFilter),
  ]);
...
export const getStudentsForExport = async (query, user = null) =>
  Student.find(await applyTeacherStudentScope(user, buildStudentFilter(query)))
    .sort({ class: 1, section: 1, rollNo: 1, _id: 1 })
    .lean();

export const importStudents = async (rows) => {
  if (!Array.isArray(rows) || rows.length === 0)
    throw new AppError("At least one student row is required.", 400);
  ...
  const students = rows.map((row, index) => {
    try {
      return normalizeAndValidateStudentPayload(row);
    } catch (error) {
      throw new AppError(`Row ${index + 2}: ${error.message}`, 400);
    }
  });
  ...
  for (const student of students) {
    ...
    if (seen.has(key))
      throw new AppError(
        `Duplicate Class ${student.class}, Section ${student.section}, Roll No ${student.rollNo} exists in the import file.`,
        409,
      );
    seen.add(key);
  }
  ...
  const existing = await Student.find({
    $or: students.map(({ class: className, section, rollNo }) => ({
      class: className,
      section,
      rollNo,
    })),
  })
    .select("class section rollNo")
    .lean();
  
  if (existing.length) {
    const first = existing[0];
    throw new AppError(
      `Roll number ${first.rollNo} already exists in Class ${first.class}, Section ${first.section}.`,
      409,
    );
  }
...
export const updateStudentById = async (
  ...
  if (
    studentData.expectedUpdatedAt &&
    new Date(studentData.expectedUpdatedAt).getTime() !==
      new Date(existing.updatedAt).getTime()
  )
    throw new AppError(
      "This student was updated by someone else. Refresh and try again.",
      409,
    );
...
export const deleteStudentById = async (studentId, requestId = null) => {
  const student = await Student.findOneAndUpdate(
    { _id: studentId, isDeleted: { $ne: true } },
...
export const bulkUpdateStudents = async ({ ids, status }, requestId = null) => {
  if (!Array.isArray(ids) || ids.length === 0 || ids.length > 200)
    throw new AppError("ids must contain between 1 and 200 student IDs.", 400);
...
export const getStudentAuditHistory = async (studentId, user = null) => {
  await assertTeacherStudentAccess(user, studentId);
  
  return getEntityAudit("student", studentId);
};
...
export const checkStudentAvailability = async ({
  ...
  if (
    ...
    throw new AppError(
      "class, section and a positive integer rollNo are required.",
      400,
    );
  }
