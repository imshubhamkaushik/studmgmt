...
const ensureIds = (ids) => {
  if (!Array.isArray(ids) || !ids.length || ids.length > 200)
    throw new AppError("studentIds must contain between 1 and 200 IDs.", 400);
  
  if (new Set(ids).size !== ids.length)
    throw new AppError("studentIds contains duplicates.", 400);
  
  if (!ids.every((id) => mongoose.isValidObjectId(id)))
    throw new AppError("studentIds contains an invalid ID.", 400);
};
...
const loadPlacement = async (academicYearId, classroomId) => {
  ...
  if (!classroom.academicYear.equals(year._id))
    throw new AppError(
      "Classroom does not belong to the selected academic year.",
      400,
    );
...
const ensureCapacity = async (classroom, additional) => {
  ...
  if (current + additional > classroom.capacity)
    throw new AppError(
      `Classroom capacity would be exceeded (${current + additional}/${classroom.capacity}).`,
      409,
    );
};
...
export const listEnrollments = async (query = {}, user = null) => {
  ...
  for (const key of ["student", "academicYear", "classroom", "status"])
    if (query[key]) filter[key] = query[key];
  
  const scopedFilter = await applyTeacherEnrollmentScope(user, filter);
  
  return Enrollment.find(scopedFilter)
    .populate("student", "studentId name status class section")
    .populate("academicYear", "name isActive")
    .populate("classroom", "className section capacity")
    .sort({ createdAt: -1 })
    .lean();
};
...
export const enrollStudent = async (data, requestId = null) => {
  ...
  if (
    ...
    !mongoose.isValidObjectId(academicYearId) ||
    !mongoose.isValidObjectId(classroomId)
  )
    throw new AppError("Invalid enrollment ID.", 400);
  if (!Number.isInteger(Number(rollNo)) || Number(rollNo) < 1)
    throw new AppError("rollNo must be a positive integer.", 400);
  ...
  const rollConflict = await Enrollment.findOne({
  ...
  if (rollConflict)
    throw new AppError(
      "Roll number is already assigned in this classroom.",
      409,
    );
  ...
  await writeAudit({
    ...
    action: "CREATE",
    changes: { after: enrollment.toObject() },
    requestId,
  });
...
export const promoteStudents = async (
  ...
  if (
    ![fromAcademicYearId, toAcademicYearId, toClassroomId].every(
  ...
  if (String(fromAcademicYearId) === String(toAcademicYearId))
    throw new AppError(
      "Source and destination academic years must differ.",
      400,
    );
  
  const { classroom } = await loadPlacement(toAcademicYearId, toClassroomId);
  
  const source = await Enrollment.find({
    student: { $in: studentIds },
    academicYear: fromAcademicYearId,
    status: "active",
  });
  
  if (source.length !== studentIds.length)
    throw new AppError(
  ...
  if (existing)
    throw new AppError(
      "At least one selected student is already enrolled in the destination academic year.",
      409,
    );
  ...
  const usedRolls = new Set(
    (
      await Enrollment.find({ classroom: toClassroomId, status: "active" })
        .select("rollNo")
        .lean()
    ).map((e) => e.rollNo),
  );
  ...
  const docs = source.map((s, index) => {
    const requested = Number(
      rollNumbers[String(s.student)] ??
        rollNumbers[s.student.toString()] ??
        index + 1,
    );
    ...
    if (
      ...
      requested < 1 ||
      usedRolls.has(requested)
    )
      throw new AppError(
        "Destination roll numbers must be positive and unique.",
        409,
      );
