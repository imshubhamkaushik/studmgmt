...
export const getAssignedClassroomIds = async (user) => {
  ...
  return TeacherClassroomAssignment.find({
    teacher: user.sub,
    isActive: true,
  }).distinct("classroom");
};
...
export const applyTeacherStudentScope = async (user, filter = {}) => {
  ...
  const rooms = await Classroom.find({ _id: { $in: ids }, isActive: true })
  ...
  if (filter.class && filter.section) {
    if (
      !pairs.some(
        (p) => p.class === filter.class && p.section === filter.section,
      )
    )
  ...
  return {
    ...filter,
    $and: [filter.$or ? { $or: filter.$or } : {}, { $or: pairs }],
  };
};
...
export const assertTeacherStudentAccess = async (user, studentId) => {
  ...
  const ok = await Enrollment.exists({
    student: studentId,
    classroom: { $in: ids },
    status: "active",
  });
...
export const applyTeacherEnrollmentScope = async (user, filter = {}) => {
  ...
  if (
    filter.classroom &&
    !ids.some((id) => String(id) === String(filter.classroom))
  )
    throw new AppError("You are not assigned to this classroom.", 403);
