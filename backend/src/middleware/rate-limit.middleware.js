...
const parseRedisUrl = (value) => {
  ...
  return { host: url.hostname, port: Number(url.port || 6379), password: url.password || null, db: url.pathname && url.pathname !== "/" ? Number(url.pathname.slice(1)) : null };
};

const encode = (parts) => `*${parts.length}\r\n${parts.map((part) => `$${Buffer.byteLength(String(part))}\r\n${part}\r\n`).join("")}`;

const redisCommand = (urlValue, parts, timeoutMs = 750) => new Promise((resolve, reject) => {
  const config = parseRedisUrl(urlValue);
  const socket = net.createConnection({ host: config.host, port: config.port });
  let buffer = "";
  const fail = (error) => { socket.destroy(); reject(error); };
  const timer = setTimeout(() => fail(new Error("Redis rate-limit request timed out.")), timeoutMs);
  const send = () => {
    const commands = [];
    if (config.password) commands.push(["AUTH", config.password]);
    if (Number.isInteger(config.db) && config.db >= 0) commands.push(["SELECT", config.db]);
    commands.push(parts);
...
const redisHit = async (key, windowMs) => {
  ...
  if (count === 1) await redisCommand(redisUrl, ["PEXPIRE", redisKey, String(windowMs)]);
  const ttl = await redisCommand(redisUrl, ["PTTL", redisKey]);
  return { count, resetAt: Date.now() + Math.max(0, Number(ttl) || windowMs) };
};

export const createRateLimiter = ({ windowMs = 60_000, max = 120, failClosed = false } = {}) => async (req, res, next) => {
  const key = req.ip || req.socket?.remoteAddress || "unknown";
  let hit;
  try {
    hit = process.env.REDIS_URL ? await redisHit(key, windowMs) : memoryHit(key, windowMs);
  } catch (error) {
    console.error(JSON.stringify({ level: "warn", event: "rate_limit_store_unavailable", message: error.message, requestId: req.requestId }));
    if (failClosed) return res.status(503).json({ success: false, message: "Rate limit service unavailable.", requestId: req.requestId });
  ...
  if (hit.count > max) return res.status(429).json({ success: false, message: "Too many requests. Please try again later.", requestId: req.requestId });
  next();
};
